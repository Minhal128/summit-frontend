...
import { CreditCard, ShieldCheck, Loader, CheckCircle, AlertCircle, Wallet } from 'lucide-react';
...
const ActivateCardPage = () => {
  ...
  const [cardData, setCardData] = useState<{ cardId: string; cardUid: string } | null>(null);
  ...
  const handleNFCScan = async () => {
    ...
    try {
      ...
      if (!('NDEFReader' in window)) {
        toast.error('NFC is not supported on this device/browser. Please use Chrome on Android or a compatible device.');
        setScanning(false);
        return;
      }
  ...
  const handleActivateCard = async () => {
    ...
    setActivating(true);
    setActivationProgress({ step: 'starting', message: 'Starting activation...', progress: 0, crypto: '' });
    ...
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/nfc-card/activate`, {
        method: 'POST',
        headers,
        body: JSON.stringify({
          ...cardData,
          ...
          {!cardData ? (
            <>
              ...
              <div style={{
                ...
                {scanning ? (
                  <Loader size={60} style={{ color: '#4CAF50', animation: 'spin 1s linear infinite' }} />
                ) : (
              ...
              <p style={{ fontSize: '16px', color: 'rgba(235, 226, 255, 0.7)', marginBottom: '40px', lineHeight: '1.6' }}>
                {scanning 
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                <button
                  ...
                  style={{
                    padding: '18px 40px',
                    background: scanning ? 'rgba(150, 150, 150, 0.3)' : 'linear-gradient(45deg, #4CAF50, #003BFC)',
                    border: 'none',
                    borderRadius: '12px',
                    color: 'white',
                    fontSize: '18px',
                    ...
                    boxShadow: scanning ? 'none' : '0 4px 20px rgba(76, 175, 80, 0.4)'
                  }}
                  onMouseEnter={(e) => {
                    if (!scanning) {
              ...
              <div style={{
                ...
                <p style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', fontSize: '13px', color: 'rgba(235, 226, 255, 0.7)' }}>
            ...
            <>
              ...
              <h2 style={{ fontSize: '28px', fontWeight: '700', marginBottom: '16px', color: '#4CAF50' }}>
                Card Detected!
              </h2>
              ...
              <div style={{
                ...
                <p style={{ fontSize: '14px', color: 'rgba(235, 226, 255, 0.6)', marginBottom: '8px' }}>Card ID:</p>
                <p style={{ fontSize: '16px', fontWeight: '600', fontFamily: 'monospace' }}>{cardData.cardId}</p>
                
                <p style={{ fontSize: '14px', color: 'rgba(235, 226, 255, 0.6)', marginTop: '16px', marginBottom: '8px' }}>Card UID:</p>
                <p style={{ fontSize: '16px', fontWeight: '600', fontFamily: 'monospace' }}>{cardData.cardUid}</p>
              </div>
              ...
              {activating && activationProgress.step && (
                <div style={{
                  ...
                  <div style={{ marginBottom: '16px' }}>
                    <p style={{ fontSize: '16px', fontWeight: '600', marginBottom: '8px', color: '#4CAF50' }}>
                      {activationProgress.message}
                    </p>
                    {activationProgress.crypto && (
                      <p style={{ fontSize: '14px', color: 'rgba(235, 226, 255, 0.7)' }}>
                  ...
                  <p style={{ fontSize: '12px', color: 'rgba(235, 226, 255, 0.6)', textAlign: 'right' }}>
                  ...
                  {activationProgress.step === 'wallet-created' && (
                    <div style={{ marginTop: '16px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                      ...
                      <span style={{ fontSize: '13px', color: 'rgba(235, 226, 255, 0.8)' }}>
                        Generated 3 {activationProgress.crypto.toUpperCase()} wallet addresses
                      </span>
                    </div>
                  )}
                </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                <button
                  ...
                  style={{
                    padding: '18px 40px',
                    background: activating ? 'rgba(150, 150, 150, 0.3)' : 'linear-gradient(45deg, #4CAF50, #003BFC)',
                    border: 'none',
                    borderRadius: '12px',
                    color: 'white',
                    fontSize: '18px',
                    ...
                    gap: '12px',
                    transition: 'all 0.3s ease',
                    boxShadow: activating ? 'none' : '0 4px 20px rgba(76, 175, 80, 0.4)',
                ...
                >
                  ...
                  {activating ? 'Generating Wallets...' : 'Activate Card & Generate Wallets'}
                </button>
